...
def compute_detection_metrics(seg_data, aruco_data):
    ...
    aruco_names = set(
        ...
        for obj in aruco_data["objects"]
    )

    seg_names = set(
        normalize_name(obj["tag"])
        for obj in seg_data.values()
    )

    seg_names = {
        x for x in seg_names
        if x not in IGNORE_LABELS
    }
    ...
    recall = (
        len(detected_gt) / len(aruco_names)
        if len(aruco_names) > 0
        else 0
    )
