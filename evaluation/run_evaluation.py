...
for i in range(1, 105):
    ...
    aruco_file = (
        ARUCO_DIR /
        f"rgb_dataset_aruco_{i}" /
        f"aruco_pos_img{i}.json"
    )
...
obj_stats.to_csv(
    OUTPUT_DIR / "object_statistics.csv",
    index=False
)
...
summary["mean_detection_recall"] = float(
    detection_df["recall"].mean()
)

summary["total_missed_objects"] = int(
    detection_df["missed_gt"].sum()
)

summary["total_extra_objects"] = int(
    detection_df["extra_objects"].sum()
)
...
plt.bar(
    obj_stats["aruco_name"],
    obj_stats["mean_error_px"]
)
...
plt.axhline(
    ...
    linestyle="--",
    linewidth=2,
    label=f"Global mean = {global_mean:.2f}px"
)
...
sorted_objects = obj_stats.sort_values(
    by="mean_error_px",
    ascending=False
)
...
plt.bar(
    sorted_objects["aruco_name"],
    sorted_objects["mean_error_px"]
)
...
plt.axhline(
    ...
    linestyle="--",
    linewidth=2,
    label=f"Mean Recall = {mean_recall:.1f}%"
)
...
plt.title(
    "Object Detection Recall per Image"
)
...
plt.savefig(
    OUTPUT_DIR /
    "detection_recall_per_image.png",
    dpi=300,
    bbox_inches="tight"
)
...
plt.title(
    "Missed and Extra Objects per Image"
)
...
depth_img_stats.to_csv(
    OUTPUT_DIR /
    "depth_error_per_image.csv",
    index=False
)
...
plt.plot(
    depth_img_stats["image"],
    depth_img_stats["mean_depth_error_mm"],
    marker="o"
)
...
plt.axhline(
    ...
    linestyle="--",
    linewidth=2,
    label=f"Global mean = {mean_depth:.2f} mm"
)

plt.axhline(
    mean_depth + global_std,
    linestyle=":",
    linewidth=1.5,
    label=f"+1σ = {global_std:.2f}"
)
...
plt.savefig(
    OUTPUT_DIR /
    "mean_depth_error_per_image.png",
    dpi=300,
    bbox_inches="tight"
)
